...
goToMainMenubtn{goToMainMenubtnPoint, buttonWidthreturn, buttonHeight, goToMainMenubtnName, ButtonFunctions::goToMainMenu}
{
    ...
    for(int y{0}; y < cell_amount; y++){
        for( int x{0}; x < cell_amount; x++){
            ...
            //cell.attach_to(*this); //this attaches every cell
...
void ChessBoard::startNewGame(){
    ...
    Rectangle erase{{0,0}, windowWidth, windowHeight};
    erase.set_fill_color(backgroundColor);
    erase.draw();
    ...
    for(auto& y : get_current_grid()){
        for(auto& c : y){
            c.updatePice();
            if(c.getPiece()){
                c.getPiece()->setEnPassent(false);
                c.getPiece()->setMoved(false);
            }
        }
    }
    updateBoard();
